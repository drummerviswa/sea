...
//------------------Customer-----------------
if (isset($_POST["customer_name"]) and isset($_POST["customer_no"])) {
    $obj = new DBOperation();
...
if (isset($_POST["manageCustomer"])) {
    $m = new Manage();
    $result = $m->manageRecordWithPagination("customers", $_POST["pageno"]);
    $rows = $result["rows"];
    $pagination = $result["pagination"];
    if (count($rows) > 0) {
        $n = (($_POST["pageno"] - 1) * 5) + 1;
        ...
        ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="#" eid="<?php echo $row['customer_id']; ?>" data-toggle="modal" data-target="#update_customer_form" class="btn btn-info btn-sm edit_customer">Edit</a>
                </td>
            </tr>
        <?php
            $n++;
...
if (isset($_POST["deleteCustomer"])) {
    $m = new Manage();
    $result = $m->deleteRecord("customers", "customer_id", $_POST["id"]);
    echo $result;
}
...
if (isset($_POST["updateCustomer"])) {
    $m = new Manage();
    $result = $m->getSingleRecord("customers", "customer_id", $_POST["id"]);
    echo json_encode($result);
    exit();
}

//Update Record after getting data
if (isset($_POST["update_product_form"])) {
    $m = new Manage();
    $id = $_POST["id"];
    $name = $_POST["customer_name"];
    $no = $_POST["customer_no"];
    $address = $_POST["address"];
    $result = $m->update_record("customers", ["customer_id" => $id], ["customer_name" => $name, "customer_no" => $no, "address" => $address]);
    echo $result;
}
...
if (isset($_POST["update_product"])) {
    ...
    $result = $m->update_record("products", ["pid" => $id], ["cid" => $cat, "bid" => $brand, "product_name" => $name, "product_price" => $price, "product_stock" => $qty, "add_date" => $date]);
    echo $result;
}
