...
class DBOperation
{
	...
	public function addProduct($cid, $bid, $pro_name, $price, $stock, $date)
	{
		$pre_stmt = $this->con->prepare("INSERT INTO `products`
			(`cid`, `bid`, `product_name`, `product_price`,
			 `product_stock`, `add_date`, `p_status`)
			 VALUES (?,?,?,?,?,?,?)");
		$status = 1;
	...
	public function addCustomer($customer_name, $customer_no, $address)
	{
		$pre_stmt = $this->con->prepare("INSERT INTO `customers`
			(`customer_name`, `customer_no`, `address`)
			 VALUES (?,?,?)");
		$pre_stmt->bind_param("sis", $customer_name, $customer_no, $address);
		$result = $pre_stmt->execute() or die($this->con->error);
		if ($result) {
